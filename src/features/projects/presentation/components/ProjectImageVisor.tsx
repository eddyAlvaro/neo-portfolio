...
const ProjectImageModal = dynamic(
  () => import("@/shared/components/ui/neon/ProjectImageModal").then((mod) => mod.ProjectImageModal),
  { ssr: false }
);
    ...
    <>
      ...
      >
        ...
        <span className="pointer-events-none absolute inset-0 z-20">
          ...
          <span className="absolute top-0 right-0 w-3 h-3 border-t-2 border-r-2 border-neon-fuchsia rounded-tr group-hover/visor:w-5 group-hover/visor:h-5 dark:group-hover/visor:shadow-[0_0_8px_rgba(232,121,249,0.7)] transition-all duration-300" />
          <span className="absolute bottom-0 left-0 w-3 h-3 border-b-2 border-l-2 border-neon-fuchsia rounded-bl group-hover/visor:w-5 group-hover/visor:h-5 dark:group-hover/visor:shadow-[0_0_8px_rgba(232,121,249,0.7)] transition-all duration-300" />
          <span className="absolute bottom-0 right-0 w-3 h-3 border-b-2 border-r-2 border-neon-cyan rounded-br group-hover/visor:w-5 group-hover/visor:h-5 dark:group-hover/visor:shadow-[0_0_8px_rgba(34,211,238,0.7)] transition-all duration-300" />
        </span>
        ...
        {project.image ? (
          ...
          <m.div
            ...
            variants={imageVariants}
            transition={{ duration: 0.35, ease: "easeOut" }}
            className="w-full h-full bg-surface/80 flex flex-col items-center justify-center gap-2 will-change-transform"
          >
            <span className="font-mono text-[22px] opacity-30 group-hover/visor:opacity-80 group-hover/visor:text-neon-cyan transition-all duration-300">
              ◈
            </span>
          </m.div>
        )}
